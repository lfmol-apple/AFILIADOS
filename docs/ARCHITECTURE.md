...
| `lib/services/monetization-score.ts` | `MonetizationScore` — "vale a pena investir tráfego nisto?", interno, nunca o `OpportunityScore` do consumidor (docs/MONETIZATION_SCORE.md)                                                          |
| `lib/services/product-matcher.ts` | `ProductMatcher` — decide se dois listings são o mesmo produto real, GTIN → fabricante → marca+modelo → textual (docs/PRODUCT_MATCHING.md)                                                              |
| `lib/services/affiliate-link-registry.ts` | Único caminho de leitura/escrita de `AffiliateLinkRegistry` — o link comercial real por listing (docs/AFFILIATE_LINK_REGISTRY.md)                                                    |
| `lib/shopee/`, `lib/providers/shopee-provider.ts` | Shopee Affiliate API (GraphQL) — assinatura, catálogo/comissão real, geração de link (docs/AFFILIATE_LINK_REGISTRY.md)                                                       |
