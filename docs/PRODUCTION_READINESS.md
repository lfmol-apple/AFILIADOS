...
| `AMAZON_BR_TRACKING_ID` | `AMAZON_BR_ASSOCIATE_TAG` is set | PreçoCaindo's own BR tag, `precocaindo-20` — never `petmol-20`. The only Amazon line that blocks `SITE_LAUNCH_READY` (a site that can't build an affiliate link at all isn't really launchable). |
...
(lint/typecheck/test/compliance/build); this script's job is "is *this
specific deployment's configuration* ready to go live", which is expected
