...
import { jaccardSimilarity, maxSimilarityAgainstCorpus, contentHash } from "@/lib/services/similarity";

describe("jaccardSimilarity", () => {
  it("returns 1 for identical text", () => {
    const text = "Este produto tem um ótimo custo benefício para quem busca praticidade.";
  ...
  it("detects near-duplicate scaled content (same template, product name swapped)", () => {
    const a = "O Fone Bluetooth XYZ tem um preço excelente comparado ao histórico coletado pelo PreçoCaindo.";
    const b = "O Air Fryer ABC tem um preço excelente comparado ao histórico coletado pelo PreçoCaindo.";
...
describe("maxSimilarityAgainstCorpus", () => {
  it("returns the highest similarity across the corpus", () => {
    const body = "Texto sobre fone de ouvido bluetooth com cancelamento de ruído.";
    const corpus = [
...
describe("contentHash", () => {
  it("is deterministic for the same normalized content", () => {
    expect(contentHash("Texto de Exemplo!")).toBe(contentHash("texto de exemplo"));
  });
