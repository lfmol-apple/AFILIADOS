...
import { slugify, slugifyWithFallback, generateUniqueSlug } from "@/lib/services/slug";
...
describe("slugifyWithFallback", () => {
  it("appends a suffix derived from the disambiguator", () => {
    expect(slugifyWithFallback("Fone Bluetooth", "B0MOCK0001")).toBe("fone-bluetooth-ck0001");
  });
});

describe("generateUniqueSlug", () => {
  it("returns the base slug when available", async () => {
    const slug = await generateUniqueSlug("Produto Novo", "B0000001", async () => false);
    expect(slug).toBe("produto-novo");
  });

  it("falls back to a disambiguated slug on collision", async () => {
    const taken = new Set(["produto-novo"]);
    const slug = await generateUniqueSlug("Produto Novo", "B0000001", async (s) => taken.has(s));
