...
export function UnifiedOfferCard({ item }: { item: UnifiedOfferCardData }) {
  ...
  return (
    <div className="group border-border-subtle bg-background flex h-full flex-col overflow-hidden rounded-xl border transition hover:shadow-md">
      <a
        href={item.href}
      ...
      {item.detailHref && (
        <a href={item.detailHref} className="text-brand hover:underline border-border-subtle border-t px-4 py-2 text-xs">
          Ver detalhes →
        </a>
      )}
    </div>
