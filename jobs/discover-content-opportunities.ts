 ...
 * job needs them.
 *
 * Intentionally hardcoded to BR, not looped over getEnabledMarketplaces()
 * like the catalog jobs — the content/editorial pipeline (GeneratedContent,
 * ContentQualityGate) has not been extended to be marketplace-aware yet
 * (see docs/CONTENT_ENGINE.md), so there is nothing for a US iteration to
 * do even once US is enabled. Revisit when content generation itself goes
 * multi-marketplace.
 */
